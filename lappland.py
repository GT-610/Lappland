...
def check_arch():
    ...
    if arch == 'aarch64' or 'armv8' in arch:
        ...
        print('Your device''s CPU architecture is not in support currently.\nWant support? Leave your needs in https://codeberg.org/GT610/lappland/issues/1 .')
        exit(1)
    return arch
...
def get_list():
    try:
        r = requests.get('https://cdn.jsdelivr.net/gh/GT-610/lappland/src/list.yaml')
...
def save_local(data):
    with open(lappland_remote, 'w') as file:
        yaml.dump(data, file)
...
def load_local(conf):
    if not os.path.isfile(lappland_conf):
        ...
        with open(lappland_conf, 'w') as f:
            yaml.dump(data, f)
...
def is_local_list_up_to_date(remote_md5,remote_sha256):
    if not os.path.isfile(lappland_remote):
        return False
    local_data = load_local(lappland_remote)
...
def show_list():
    if os.path.isfile(lappland_remote):
        with open(lappland_remote, 'r') as file:
    ...
    config = load_local(lappland_conf)
    table = PrettyTable()
    arch = check_arch()
